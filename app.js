...
async function writeWordsfast(words) {
    for (let char of words) {
        process.stdout.write(char);
        await new Promise((resolve) => {
...
class patientsInfo {
    ...
    age;
    contactNumber;
    constructor(name, fatherName, gender, age, contactNumber) {
        this.name = name,
...
async function main() {
    ...
    await writeWordsfast(chalk.greenBright(`\n\t\t ${patientDetails.name.split(" ").map((a) => a.charAt(0).toUpperCase() + a.slice(1).toLowerCase()).join()}\n`));
    await writeWordsfast(chalk.greenBright(`\n\t\t ${patientDetails.fatherName.split(" ").map((a) => a.charAt(0).toUpperCase() + a.slice(1).toLowerCase()).join(" ")}\n`));
